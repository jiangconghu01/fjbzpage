...
export default {
  ...
  computed: {
    ...mapGetters(['month', 'orgCode', 'authCityLevel', 'listAuth']),
  },
  watch: {
    month(nv, ov) {
      this.initpage()
  ...
  methods: {
    ...
    top(month, code, type) {
      ...
      const c_code = this.listAuth.map((val) => val.value)
      const c_Name = this.listAuth.map((val) => val.orgName)
      //   const cityArr = [code, ...c_code]
      //   const xArr = ['全省', ...c_Name]
      const cityArr = c_code
      const xArr = c_Name.map((val) => {
        if (val === '福建') {
          return '全省'
        } else {
          return val
      ...
      this.$http.post('/channelBigScreen/modIdxVOList', param).then((res) => {
        ...
        encode.forEach((ele, i) => {
          ...
          s.label.formatter = function (p) {
            ...
            return label === '' ? '' : formatNumberRgx(label)
          }
